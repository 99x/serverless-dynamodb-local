...
class ServerlessDynamodbLocal {
          ...
          start: {
            ...
            options: {
              ...
              graceful: {
                shortcut: "g",
                usage:
                  "While creating DynamoDB tables do not exit with error on pre-exiting tables"
              },
              seed: {
                shortcut: "s",
                usage:
  ...
  startHandler() {
    ...
    return BbPromise.resolve()
      .then(() => options.migrate && this.migrateHandler(!!options.graceful))
  ...
  createTable(dynamodb, migration, graceful = false) {
    return new BbPromise((resolve, reject) => {
      ...
      dynamodb.raw.createTable(migration, err => {
        if (err) {
          this.serverlessLog(`DynamoDB - Error - ${err.toString()}`);
          if (graceful) {
            resolve(err);
          } else {
            reject(err);
